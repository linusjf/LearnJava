...
public class RequestProcessor implements Runnable {
  ...
  public RequestProcessor(File rootDirectory, String indexFileName, Socket connection) {
    ...
    if (root == null) this.rootDirectory = rootDirectory;
    else this.rootDirectory = root;
  ...
  public void run() {
    try (OutputStream raw = new BufferedOutputStream(connection.getOutputStream());
        ...
        Reader in =
            new InputStreamReader(new BufferedInputStream(connection.getInputStream()), US_ASCII);
        connection; ) {
      ...
      if ("GET".equals(method)) handleGet(tokens, raw, out);
      else {
  ...
  private String getVersionFromTokens(String... tokens) {
    if (tokens.length > (1 + 1)) return tokens[2];
    return "";
  }
