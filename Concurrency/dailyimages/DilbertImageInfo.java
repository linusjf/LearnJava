...
class DilbertImageInfo extends ImageInfo {
  ...
  private static String findProperty(String body, String filter) {
    ...
    return body.lines().filter(line -> line.contains(search)).findFirst().map(line -> line.replaceAll(".*" + search, "")).map(line -> line.replaceAll("\".*", "")).orElseThrow(() -> new IllegalStateException("Could not find \"" + filter + "\""));
  }
