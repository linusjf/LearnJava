...
public class GrayscaleImageAction extends RecursiveAction {
  ...
  public GrayscaleImageAction(int row,
  ...
  protected void compute() {
    for (int column = 0; column < bufferedImage.getWidth();
         column++) {
      int rgb = bufferedImage.getRGB(column, row);
      int r = (rgb >> 16) & 0xFF;
      int g = (rgb >> 8) & 0xFF;
      int b = rgb & 0xFF;
      int gray = (int)(0.2126 * (float)r + 0.7152 * (float)g
                       + 0.0722 * (float)b);
  ...
  public static void main(String[] args) {
    try {
      ...
      for (int row = 0; row < height; row++) {
        GrayscaleImageAction action =
            new GrayscaleImageAction(row, img);
        pool.execute(action);
      }
      pool.shutdown();
      pool.awaitTermination(1, TimeUnit.DAYS);
      final long endTime = System.currentTimeMillis();
      System.out.println("Image graying took "
                         + (endTime - startTime)
                         + " milliseconds.");
